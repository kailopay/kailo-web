...
import { PaymentLinksFilters } from "@/components/dashboard/ui/payment-links/use-payment-link-filters";
import { PaymentLinksTableSkeleton } from "@/components/dashboard/ui/payment-links/payment-links-table-skeleton";
...
import { Copy, Dots, Hyperlink, Trash } from "@dub/ui/icons";
...
export function PaymentLinksTable({
  ...
  const { columnVisibility, setColumnVisibility } = useColumnVisibility(
    "kailopay-payment-links-table-columns",
    paymentLinkColumns,
  );
      ...
      },
      {
        ...
        cell: ({ row }: { row: Row<PaymentLinkRow> }) => (
          <RowMenuButton
            ...
            onDeleted={() => setLocalRefreshKey((key) => key + 1)}
          />
        ),
      },
    ],
...
function RowMenuButton({
  ...
  async function handleDelete() {
    const response = await fetch(
      `/api/organizations/${organizationId}/payment-links/${row.original.id}`,
      { method: "DELETE" },
    );

    if (!response.ok) {
      const data = (await response.json()) as { error?: string };
      throw new Error(data.error ?? "Unable to delete payment link");
    }
  }
            ...
            <MenuItem
              ...
              onSelect={() => {
                ...
                toast.promise(handleDelete(), {
                  loading: "Deleting payment link...",
                  success: () => {
                    onDeleted();
                    return "Payment link deleted";
                  },
                  error: (err) =>
