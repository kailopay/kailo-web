...
import { SETTLEMENT_FALLBACK_TYPE_LABEL } from "@/lib/dashboard/payments/settlement/fallback";
...
import { SettlementsFilters } from "@/components/dashboard/ui/settlements/use-settlement-filters";
import { SettlementsTableSkeleton } from "@/components/dashboard/ui/settlements/settlements-table-skeleton";
...
export function SettlementsTable({
  ...
  const { columnVisibility, setColumnVisibility } = useColumnVisibility(
    "kailopay-settlements-table-columns",
    settlementsColumns,
  );
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        minSize: 120,
        cell: ({ row }: { row: Row<SettlementConversionRow> }) =>
          row.original.settlement_fallback
            ? SETTLEMENT_FALLBACK_TYPE_LABEL
            : row.original.converted_on_chain
              ? "Path payment"
              : "Direct receive",
      },
      {
