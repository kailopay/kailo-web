...
import { AssetAmountCell } from "@/components/dashboard/ui/assets/asset-amount-cell";
import { PaidAmountCell } from "@/components/dashboard/ui/payments/paid-amount-cell";
import {
  CONFIDENTIAL_PAYER_DESCRIPTION,
  formatPayerDisplay,
  getMerchantSettlementDisplay,
...
import { TransactionsFilters } from "@/components/dashboard/ui/transactions/use-transaction-filters";
...
export function TransactionsTable({
  ...
  const { columnVisibility, setColumnVisibility } = useColumnVisibility(
    "kailopay-transactions-table-columns",
    transactionsColumns,
  );
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        minSize: 120,
        cell: ({ row }: { row: Row<PaymentRow> }) => {
          const settlement = getMerchantSettlementDisplay(row.original);
      ...
      },
      {
        ...
        minSize: 140,
        cell: ({ row }: { row: Row<PaymentRow> }) =>
          isConfidentialPayment(row.original) ? (
            <span
              className="text-xs text-neutral-500"
              title={CONFIDENTIAL_PAYER_DESCRIPTION}
            >
              {formatPayerDisplay(row.original)}
            </span>
      ...
      content={
        <Command tabIndex={0} loop className="focus:outline-none">
          <Command.List className="flex w-screen flex-col gap-1 p-1.5 text-sm focus-visible:outline-none sm:w-auto sm:min-w-[160px]">
            ...
            {row.original.payer_address && !isConfidentialPayment(row.original) ? (
              <MenuItem
                as={Command.Item}
                icon={Copy}
                onSelect={() => {
