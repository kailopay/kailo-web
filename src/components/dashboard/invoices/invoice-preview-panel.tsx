...
import { InvoiceDocument } from "@/components/dashboard/invoices/invoice-document";
import { BrowserPreviewFrame } from "@/components/dashboard/ui/invoices/browser-preview-frame";
import { getInvoiceCheckoutPreviewUrl } from "@/lib/dashboard/invoices/preview-url";
import type { InvoicePresentation } from "@/lib/dashboard/invoices/presentation";
import { cn } from "@dub/utils";
import type { CheckoutData } from "@/components/dashboard/checkout/checkout-types";
import { CheckoutView } from "@/components/dashboard/checkout/checkout-view";
...
export function InvoicePreviewPanel({ organizationId, presentation, environment }: { organizationId: string; presentation: InvoicePresentation; environment: "sandbox" | "production" }) {
  ...
  const [previewPaymentMethod, setPreviewPaymentMethod] = useState<"qr" | "wallet">("qr");
  const [previewUsePrivatePayment, setPreviewUsePrivatePayment] = useState(false);
  ...
  useEffect(() => {
    ...
    fetch(`/api/organizations/${organizationId}/invoices/preview-email`, {
      ...
      .catch((error: unknown) => {
        if (active) {
          setEmailError(error instanceof Error ? error.message : "Unable to load email preview");
        }
      });
  ...
  const checkoutPreviewUrl = useMemo(() => {
    return getInvoiceCheckoutPreviewUrl(presentation, previewOrigin ?? undefined);
  ...
  const mockCheckoutData = useMemo<CheckoutData>(() => {
    ...
    const selectedPaidAsset = allowedAssets.find((asset) => `${asset.asset_code}:${asset.issuer_address ?? ""}` === selectedPaidAssetKey) ?? allowedAssets[0]!;
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="mb-4">
        ...
        <p className="text-content-subtle text-xs">{environment === "production" ? "Production invoice" : "Test invoice"}</p>
      </div>
      ...
      <div className="mb-4 flex items-center justify-between">
        <div className="flex gap-1">
          {(
            ...
            <button key={value} type="button" onClick={() => setTab(value)} className={cn("rounded-md px-3 py-1.5 text-sm font-medium transition-colors", tab === value ? "bg-bg-muted text-content-default" : "text-content-subtle hover:text-content-default")}>
              {label}
            </button>
          ))}
        </div>
        ...
        {tab === "payment" ? (
          <div className="flex items-center gap-1 rounded-lg border border-neutral-200 bg-neutral-50 p-1">
            <button type="button" onClick={() => setPreviewDevice("responsive")} className={cn("rounded-md p-1 transition-all", previewDevice === "responsive" ? "bg-white text-neutral-900 shadow-sm" : "text-neutral-400 hover:text-neutral-600")} title="Desktop (Responsive)">
              <Monitor className="size-4" />
            </button>
            <button type="button" onClick={() => setPreviewDevice("mobile")} className={cn("rounded-md p-1 transition-all", previewDevice === "mobile" ? "bg-white text-neutral-900 shadow-sm" : "text-neutral-400 hover:text-neutral-600")} title="Mobile">
      ...
      <div className="bg-bg-preview min-h-0 flex-1 overflow-hidden rounded-lg p-3">
        ...
        {tab === "email" ? <div className="flex h-full flex-col overflow-hidden rounded-md border border-neutral-200 bg-white">{emailHtml ? <iframe title="Invoice email preview" srcDoc={emailHtml} className="h-full min-h-0 w-full flex-1 border-0" /> : emailError ? <div className="text-content-subtle flex min-h-[200px] flex-1 items-center justify-center text-sm">{emailError}</div> : <div className="text-content-subtle flex min-h-[200px] flex-1 items-center justify-center text-sm">Loading email preview...</div>}</div> : null}

        {tab === "payment" ? (
          <BrowserPreviewFrame url={checkoutPreviewUrl} className={cn("h-full mx-auto transition-[width] duration-500 ease-in-out", previewDevice === "mobile" ? "w-[375px]" : "w-full")}>
            <CheckoutView
                ...
                qrDestination="G..."
                checkoutQrValue="web+stellar:pay?destination=G...&amount=0"
                ...
                onPaymentMethodChange={setPreviewPaymentMethod}
                usePrivatePayment={previewUsePrivatePayment}
                onUsePrivatePaymentChange={setPreviewUsePrivatePayment}
                disabled={true}
                countdown="02:45"
              />
          </BrowserPreviewFrame>
