...
import { readJsonResponse } from "@/lib/dashboard/http/read-json-response";
import { getPaymentsHubHref } from "@/lib/dashboard/navigation/payments-tabs";
import type { PaymentLinkRow } from "@/lib/dashboard/payments/types";
import { PaymentLinkDetailSkeleton } from "@/components/dashboard/ui/payments/payment-link-detail-skeleton";
import { PaymentLinkDetailStats } from "@/components/dashboard/ui/payments/payment-link-detail-stats";
import { PaymentLinkDetailsColumn } from "@/components/dashboard/ui/payments/payment-link-details-column";
import {
  formatPaymentLinkAmount,
  getPaymentLinkStatusVariant,
} from "@/components/dashboard/ui/payments/payment-formatters";
import {
  PaymentLinkCustomerCollectionSection,
  PaymentLinkMetadataSection,
  PaymentLinkProductsSection,
} from "@/components/dashboard/ui/payments/payment-link-sections";
import { ShareLinkSection } from "@/components/dashboard/ui/payments/share-link-section";
...
import { ChevronRight, Copy, Dots, Hyperlink, Link4, Trash } from "@dub/ui/icons";
...
export function PaymentLinkDetailPanel({
  ...
  const fetchLink = useCallback(async () => {
    ...
    const data = await readJsonResponse<PaymentLinkRow & { error?: string }>(
      response,
    );
  ...
  const deletePaymentLink = useCallback(async () => {
    ...
    if (!response.ok) {
      toast.error(data.error ?? "Unable to delete payment link");
      return;
    }

    toast.success("Payment link deleted");
    router.push(getPaymentsHubHref("payment-links"));
  }, [organizationId, linkId, router]);
  ...
  const headerOverride = useMemo(() => {
    ...
    return {
      ...
      controls: (
        <PaymentLinkDetailMenu
          link={link}
          onDelete={deletePaymentLink}
        />
      ),
    };
  }, [deletePaymentLink, headerTitle, link]);
...
function PaymentLinkDetailMenu({
  link,
  onDelete,
}: {
  link: PaymentLinkRow;
