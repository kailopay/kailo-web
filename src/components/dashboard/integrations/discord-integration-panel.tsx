...
import type { NotificationIntegrationSettings, PublicDiscordIntegration, PublicOrganizationIntegration } from "@/lib/dashboard/integrations/types";
...
function parseSettings(integration: PublicOrganizationIntegration | null): NotificationIntegrationSettings {
  ...
  if (!settings || !Array.isArray(settings.events)) {
    return {
      ...
      templates: getDefaultTemplatesForProvider("discord"),
    };
  }
  ...
  return {
    ...
    templates: {
      ...getDefaultTemplatesForProvider("discord"),
      ...(settings.templates ?? {}),
    },
    channelLabel: settings.channelLabel,
    discordDelivery: settings.discordDelivery,
    discordChannelId: settings.discordChannelId,
    discordChannelName: settings.discordChannelName,
...
function isBotInstallationReady(
  ...
  const credentials = integration.credentials as PublicDiscordIntegration | undefined;
  return Boolean(
    credentials?.delivery === "bot" && credentials.installationReady,
  );
}

export function DiscordIntegrationPanel({ organizationId }: { organizationId: string }) {
  ...
  const headerOverride = useMemo(
    () => ({
      titleInfo: {
        title: "Send payment notifications to a Discord channel with the bot.",
      },
    }),
    [],
  );
  ...
  const applySettings = useCallback((nextIntegration: PublicOrganizationIntegration | null) => {
    ...
    setGuildName(settings.discordGuildName ?? null);
  }, []);
  ...
  const loadIntegration = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`/api/organizations/${organizationId}/integrations/discord`);
      ...
      if (!response.ok) {
        throw new Error(data.error ?? "Unable to load Discord integration");
      }
  ...
  const loadChannels = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/organizations/${organizationId}/integrations/discord/channels`);
      const data = (await response.json()) as {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error ?? "Unable to load Discord channels");
      }
      ...
      setChannels([]);
      setChannelsError(error instanceof Error ? error.message : "Unable to load Discord channels");
    } finally {
  ...
  useEffect(() => {
    ...
    if (isBotInstallationReady(integration) && settings.discordGuildId) {
      void loadChannels();
    }
  ...
  useEffect(() => {
    ...
    if (connected === "1") {
      toast.success("Bot added to your Discord server");
      void loadIntegration();
      router.replace("/dashboard/integrations/discord");
      return;
    }
    ...
    void (async () => {
      try {
        ...
        toast.error(data.integration?.lastError ?? "Unable to connect Discord. Try adding the bot again.");
        setIntegration(data.integration ?? null);
        applySettings(data.integration ?? null);
      } catch {
        toast.error("Unable to connect Discord");
      } finally {
        router.replace("/dashboard/integrations/discord");
      }
    })();
  ...
  async function handleAddToDiscord() {
    ...
    try {
      const response = await fetch(`/api/organizations/${organizationId}/integrations/discord/connect`, { method: "POST" });
      const data = (await response.json()) as {
      ...
      toast.error(error instanceof Error ? error.message : "Unable to start Discord connection");
  ...
  async function handleCompleteSetup() {
    ...
    try {
      const response = await fetch(`/api/organizations/${organizationId}/integrations/discord/complete`, {
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(data.error ?? "Unable to complete Discord setup");
      }
      ...
      toast.error(error instanceof Error ? error.message : "Unable to complete Discord setup");
    } finally {
  ...
  async function handleTest(event: string, template: string) {
    ...
    try {
      const response = await fetch(`/api/organizations/${organizationId}/integrations/discord/test`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleDisconnect() {
    ...
    try {
      const response = await fetch(`/api/organizations/${organizationId}/integrations/discord`, { method: "DELETE" });
      const data = (await response.json()) as { error?: string };

      if (!response.ok) {
        throw new Error(data.error ?? "Unable to disconnect Discord");
      }

      toast.success("Discord disconnected");
      resetLocalIntegrationState();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Unable to disconnect Discord");
    } finally {
  ...
  const hasGuild = isBotInstallationReady(integration) && Boolean(settings.discordGuildId);
  const isPendingSetup = integration?.status === "pending" && hasGuild;
  const publicDiscord = integration?.credentials && "delivery" in integration.credentials ? (integration.credentials as PublicDiscordIntegration) : null;

  const hasChannelChange =
    isConnected &&
    Boolean(selectedChannelId) &&
    selectedChannelId !== (settings.discordChannelId ?? "");
  ...
  const sectionAction = isConnected ? (
    ...
    <Button type="button" text="Add to Discord" className="h-9 w-fit shrink-0" loading={isConnecting} onClick={() => void handleAddToDiscord()} />
  );
  ...
  return (
    <div className="mb-6 space-y-6">
      <SettingsSection
        title="Discord bot"
        description="Add the bot to your server, then choose which channel receives payment notifications."
        helpText={
          isConnected ? (
            <p className="text-sm text-neutral-500">
              Connected to <span className="text-neutral-900">{publicDiscord?.guildName ?? guildName ?? settings.discordGuildName}</span>
              {publicDiscord?.channelName || settings.discordChannelName ? (
                <>
                  {" "}
                  · <span className="text-neutral-900">#{publicDiscord?.channelName ?? settings.discordChannelName}</span>
                </>
              ) : null}
              . Pick another channel below and click <span className="text-neutral-900">Save changes</span> to move notifications.
            </p>
          ) : isPendingSetup ? (
            <p className="text-sm text-neutral-500">
              Bot added to your server. Pick a notification channel, then click <span className="text-neutral-900">Complete setup</span> to finish. Use <span className="text-neutral-900">Disconnect</span> to switch to a different server.
            </p>
          ) : (
            <p className="text-sm text-neutral-500">
              You need Manage Server permission to add the bot.{" "}
              <Link href="/guides/integrations/discord" className="underline underline-offset-4 hover:text-neutral-700">
                Setup guide
              </Link>
            </p>
          )
        ...
        <div className="space-y-4">
          ...
          {!hasGuild ? (
            <p className="text-sm text-neutral-500">
              Click <span className="text-neutral-900">Add to Discord</span> to invite the bot, then pick a channel below.
            </p>
          ) : (
            <div className="max-w-md space-y-2">
              <label htmlFor="discord-channel" className="text-sm font-medium text-neutral-900">
                Notification channel
              </label>
              {isChannelsLoading ? (
                <SmoothSkeleton className="h-10 w-full rounded-md" />
              ) : (
                <select id="discord-channel" className="h-10 w-full rounded-md border border-neutral-300 bg-white px-3 text-sm text-neutral-900 focus:border-neutral-500 focus:outline-none focus:ring-neutral-500" value={selectedChannelId} onChange={(event) => setSelectedChannelId(event.target.value)} disabled={channels.length === 0}>
                  {channels.length === 0 ? (
              ...
              {guildName || settings.discordGuildName ? (
                <p className="text-sm text-neutral-500">
                  Server: <span className="text-neutral-900">{guildName ?? settings.discordGuildName}</span>
                </p>
              ) : null}
            </div>
          )}
      ...
      <NotificationEventsSettings
        ...
        variablesDocsHref="/guides/integrations/discord#message-variables"
        defaultTemplates={getDefaultTemplatesForProvider("discord")}
