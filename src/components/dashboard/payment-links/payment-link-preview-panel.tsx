...
import { getPaymentLinkCheckoutPreviewUrl } from "@/lib/dashboard/payment-links/preview-url";
import type { PaymentLinkCustomerInput, PaymentLinkPresentation } from "@/lib/dashboard/payment-links/types";
...
export function PaymentLinkPreviewPanel({ presentation, environment }: { presentation: PaymentLinkPresentation; environment: "sandbox" | "production" }) {
  ...
  const [previewCustomerInput, setPreviewCustomerInput] = useState<PaymentLinkCustomerInput>({});
  ...
  const checkoutPreviewUrl = useMemo(() => getPaymentLinkCheckoutPreviewUrl(previewOrigin ?? undefined), [previewOrigin]);

  const mockCheckoutData = useMemo<CheckoutData>(() => {
    const allowedAssets = [
      {
        asset_code: "USDC",
        issuer_address: "GBBD47IF6LWK7P7MDEVFA4DJ7ZTLZ5SSSU3PG3TK23OJZ7CTYYIB6W2U",
      },
    ...
    return {
      ...
      merchant_memo: presentation.description?.trim() || null,
  ...
  const hasPricing = Boolean(presentation.currencyCode && presentation.currencyCode !== mockCheckoutData.payment.settlement_asset.asset_code);

  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="mb-4">
        <p className="text-content-default text-sm font-medium">Preview</p>
        <p className="text-content-subtle text-xs">{environment === "production" ? "Production payment link" : "Test payment link"}</p>
      </div>
      ...
      <div className="bg-bg-preview min-h-0 flex-1 overflow-hidden rounded-lg p-3">
        <BrowserPreviewFrame url={checkoutPreviewUrl} className={cn("mx-auto h-full transition-[width] duration-500 ease-in-out", previewDevice === "mobile" ? "w-[375px]" : "w-full")}>
          <CheckoutView
