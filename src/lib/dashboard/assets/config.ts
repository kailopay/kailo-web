...
import type { AllowedAsset, AssetConfig } from "@/lib/dashboard/assets/types";
...
export async function getOrganizationDefaultAssetConfig(
  ...
  if (enabled.length === 0) {
    throw new Error("No enabled payment methods. Configure them in Settings → Settlement Wallet.");
  }
