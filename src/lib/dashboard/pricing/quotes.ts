...
import {
  ...
  RATE_CACHE_TTL_MS,
  STABLECOIN_FIAT_MAP,
} from "@/constants/dashboard/pricing/quotes";
import {
...
export function isAmountUnderpaid(
  expected: string,
  received: string,
  slippageBps = DEFAULT_SLIPPAGE_BPS,
) {
  ...
  if (expectedNum <= 0) {
    return receivedNum < 0;
  }

  const minAcceptable = expectedNum * (1 - slippageBps / 10_000);
  return receivedNum < minAcceptable;
}
