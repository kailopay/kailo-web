...
import { WEBHOOK_TIMESTAMP_TOLERANCE_SECONDS } from "@/constants/dashboard/webhooks/retry";
...
export function buildWebhookHeaders(input: {
  ...
  return {
    ...
    "KailoPay-Signature": signature,
    "KailoPay-Event": input.event,
    "KailoPay-Timestamp": String(timestamp),
    "KailoPay-Delivery-ID": input.deliveryId,
  };
}
...
export function verifyKailoPayWebhookSignature(input: {
  secret: string;
  rawBody: string;
