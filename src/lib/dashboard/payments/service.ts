...
import { customers, organizations, payments, type Organization, type Payment } from "@/lib/dashboard/db/schema";
import type { AllowedAsset } from "@/lib/dashboard/assets/types";
import { resolveAssetConfig } from "@/lib/dashboard/assets/config";
import {
  dbAllowedAssets,
  serializePaymentAssets,
} from "@/lib/dashboard/assets/serialize";
import { organizationEnvironmentWhere } from "@/lib/dashboard/organizations/environment-scope";
import { requireSettlementWallet } from "@/lib/dashboard/organizations/settlement-wallet";
import { getCustomerByPublicId } from "@/lib/dashboard/customers/service";
import type { InvoiceCurrencyCode } from "@/lib/dashboard/invoices/currencies";
import { buildPaymentQuote } from "@/lib/dashboard/pricing/quotes";
import { normalizeStellarAmount } from "@/lib/dashboard/stellar/amount";
import { resolvePaymentSettlementTxHash } from "@/lib/dashboard/payments/settlement/tx-hash";
import { resolveStellarTransactionHash } from "@/lib/dashboard/stellar/transaction-hash";
import {
  getEscrowDepositAddress,
} from "@/lib/dashboard/stellar/escrow/config";
import {
  getSorobanConfig,
} from "@/lib/dashboard/soroban/config";
import { assertSorobanConfigured } from "@/lib/dashboard/soroban/setup-errors";
...
import { getAppUrl } from "@/constants/dashboard/app";
import {
...
export async function createPayment(input: {
  ...
  const [payment] = await db
    ...
    .values({
      ...
      paymentAuthorizationHash: null,
      shieldedPoolContractId: null,
  ...
  const [updated] = await db
    ...
    .set({
      ...
      metadata: {
        ...
        escrow_deposit_channel: "contract",
      },
      updatedAt: new Date(),
    })
...
export async function updatePaymentStatus(
  ...
  if (status in eventMap && !options?.suppressWebhook) {
    await dispatchWebhookEvent({
...
export function serializePayment(
  ...
  return {
    ...
    tx_hash: resolvePaymentSettlementTxHash(payment),
