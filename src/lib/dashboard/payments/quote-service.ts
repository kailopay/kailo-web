...
import { allowedAssetsEquivalent, findAllowedAsset, resolveAllowedAsset } from "@/lib/dashboard/assets/types";
import type { AllowedAsset } from "@/lib/dashboard/assets/types";
import { db } from "@/lib/dashboard/db";
import { invoices, payments, type Payment } from "@/lib/dashboard/db/schema";
import { isInvoiceCurrencyCode } from "@/lib/dashboard/invoices/currencies";
import { getInvoiceCheckoutSessionExpiredMessage } from "@/lib/dashboard/invoices/status";
import { DEFAULT_PAYMENT_SESSION_HOURS } from "@/constants/dashboard/payments/defaults";
import { buildPaymentQuote, isQuoteExpired } from "@/lib/dashboard/pricing/quotes";
import { applyPaymentQuote, setPaymentPaidAsset, updatePaymentStatus } from "@/lib/dashboard/payments/service";
import { isCheckoutProcessingStatus } from "@/lib/dashboard/checkout/payment-state";
import { isRetryableFailedPayment, resetPaymentForRetry } from "@/lib/dashboard/payments/retry";
import { processEscrowSettlement } from "@/lib/dashboard/payments/settlement/escrow";
import { ensureEscrowPaymentRegistered } from "@/lib/dashboard/soroban/escrow-contract";
import { syncEscrowOperatorTrustlines } from "@/lib/dashboard/stellar/escrow/operator-trustlines";
...
export function needsPaymentQuoteRefresh(payment: Payment, paidAsset: AllowedAsset) {
  ...
  return !payment.quotedPaidAmount || !payment.quoteExpiresAt || isQuoteExpired(payment.quoteExpiresAt) || assetChanged;
}
...
async function resolveInvoiceSessionExpiry(payment: Payment) {
  ...
  return new Date(payment.createdAt.getTime() + DEFAULT_PAYMENT_SESSION_HOURS * 60 * 60 * 1000);
}
...
async function resolveInvoicePaymentSessionExpiredError(payment: Payment) {
  ...
  const [invoice] = await db.select({ dueAt: invoices.dueAt }).from(invoices).where(eq(invoices.id, payment.invoiceId)).limit(1);
...
export async function ensurePayablePayment(payment: Payment): Promise<{ payment: Payment; error?: string }> {
...
function resolveQuotePaidAsset(payment: Payment, paidAsset: AllowedAsset) {
  if (!payment.pricingCurrency || !payment.pricingAmount) {
  ...
  const match = findAllowedAsset(allowed, paidAsset.asset_code, paidAsset.issuer_address, payment.environment);
...
export async function previewPaymentQuote(payment: Payment, paidAsset: AllowedAsset) {
  ...
  return buildPaymentQuote({
    ...
    pricingCurrency: payment.pricingCurrency as Parameters<typeof buildPaymentQuote>[0]["pricingCurrency"],
    paidAsset: match,
    settlementAsset,
  });
}

export async function refreshPaymentQuote(payment: Payment, paidAsset: AllowedAsset) {
  const { canonicalPaidAsset, settlementAsset, match } = resolveQuotePaidAsset(payment, paidAsset);
...
export async function ensurePaymentSettlementQuote(
  payment: Payment,
  paidAssetInput?: AllowedAsset,
) {
  const resolvedPaidAsset: AllowedAsset | null = paidAssetInput
    ? resolveAllowedAsset(paidAssetInput, payment.environment)
    : payment.paidAsset
      ? resolveAllowedAsset(
          {
            asset_code: payment.paidAsset,
  ...
  if (!payment.pricingCurrency || !payment.pricingAmount || !resolvedPaidAsset) {
    return payment;
  }
  ...
  const crossAsset = !allowedAssetsEquivalent(
    resolvedPaidAsset,
    settlementAsset,
    payment.environment,
  );
  ...
  if (
    ...
    !needsPaymentQuoteRefresh(payment, resolvedPaidAsset)
  ) {
    return payment;
  }

  if (crossAsset && !payment.quotedSettlementAmount) {
    const { payment: updated } = await refreshPaymentQuote(
      payment,
      resolvedPaidAsset,
    );
    return updated;
  ...
  if (!needsPaymentQuoteRefresh(payment, resolvedPaidAsset)) {
    return payment;
  }
