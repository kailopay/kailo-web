...
import { resolveAllowedAsset, type AllowedAsset } from "@/lib/dashboard/assets/types";
import type { Payment } from "@/lib/dashboard/db/schema";
import { recordEscrowContractDepositReceived } from "@/lib/dashboard/payments/service";
import { assetsMatch } from "@/lib/dashboard/pricing/quotes";
...
import { REFUND_REASONS, type RefundReason } from "@/lib/dashboard/payments/settlement/constants";
...
export async function registerEscrowPaymentOnContract(payment: Payment) {
  ...
  const settlementAmount =
    ...
    (!crossAsset ? payment.merchantSettlementAmount ?? requiredAmount : null);

  if (!settlementAmount) {
    throw new Error(
      "Settlement quote is required before registering cross-asset payment",
    );
  }
