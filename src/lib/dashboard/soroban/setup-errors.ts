...
import { isStellarOperatorConfigured } from "@/lib/dashboard/stellar/operator";
...
export function sorobanSetupSteps(
  ...
  return [
    `Fund a Stellar ${environment === "production" ? "Mainnet" : "Testnet"} account for the KailoPay operator.`,
    ...
    `Deploy and initialize the KailoPay Soroban contract, then set SOROBAN_${prefix}_CONTRACT_ID.`,
...
export function classifySorobanSimulationError(
  ...
  if (normalized.includes("notinitialized") || normalized.includes("#2")) {
    return createSorobanSetupError({
      ...
      environment,
      message:
        "The KailoPay Soroban contract is not initialized. Run initialize(admin, authorization_signer, fee_recipient) on the deployed contract.",
      setup: [
  ...
  if (normalized.includes("unauthorized") || normalized.includes("#3")) {
    return createSorobanSetupError({
      ...
      environment,
      message:
        "The operator account is not authorized on the KailoPay Soroban contract. The contract authorization_signer must match STELLAR_*_OPERATOR_SECRET.",
      setup: [
        ...sorobanSetupSteps(environment),
  ...
  if (normalized.includes("paused") || normalized.includes("#4")) {
    return createSorobanSetupError({
      ...
      environment,
      message: "The KailoPay Soroban contract is paused.",
      setup: [
  ...
  if (
    normalized.includes("proofinvalid") ||
    normalized.includes("error(contract, #9)") ||
    ...
    return createSorobanSetupError({
      ...
      environment,
      message:
        "The zero-knowledge proof was rejected by the shielded pool contract. Refresh the checkout page, confirm your deposit, and generate a new proof.",
      setup: [
        "Ensure withdraw uses the same shielded pool contract as your deposit.",
        "Confirm the contract verifying key matches apps/web/public/zk/verification_key.json; rebuilding the circuit invalidates older keys.",
        "Hard-refresh checkout and regenerate the proof after confirming the deposit.",
        ...sorobanSetupSteps(environment),
      ],
    });
  }
  ...
  if (
    normalized.includes("error(budget") ||
    normalized.includes("budget, exceededlimit")
  ) {
    return createSorobanSetupError({
      code: "simulation_failed",
      environment,
      message:
        "The private payment withdraw exceeded the Soroban CPU budget. Proof verification normally uses about a third of the ledger limit, so this points at an outdated shielded pool contract rather than at your payment.",
      setup: [
        "Confirm the deployed shielded pool runs the current contract build; proof verification must use the native BN254 host functions.",
        "Redeploy with ./scripts/soroban-deploy-shielded-pool.sh, or call upgrade with the new WASM hash to keep pooled deposits.",
        "Re-run node scripts/verify-shielded-withdraw-e2e.mjs to confirm the deployed contract verifies a real proof.",
        ...sorobanSetupSteps(environment),
      ],
    });
  }
